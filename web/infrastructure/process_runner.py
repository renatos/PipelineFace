...
class AsyncProcessRunner:
    ...
    def run_process_async(self, command: List[str], process_name: str, on_complete_callback=None):
        with self._lock:
            if self.active_process["running"]:
                self.active_process["logs"].append(f"⚠️ {process_name} não iniciado: já existe um processo em execução.")
                return
    ...
    def terminate_process(self) -> Dict[str, str]:
        ...
        try:
            # Mata o grupo de processos inteiro (python pai + filhos Chromium/Playwright/ffmpeg)
            try:
