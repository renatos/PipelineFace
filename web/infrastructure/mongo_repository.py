...
class MongoStrategyRepository(AbstractStrategyRepository):
    def __init__(self, mongo_uri: str = "mongodb://localhost:27017", db_name: str = "pipelineface", collection_name: str = "seo_knowledge"):
        ...
        # Índice único em basename (protegido para não derrubar a app se houver duplicados)
        try:
            self.collection.create_index("basename", unique=True)
        except Exception as e:
            logger.warning(f"Não foi possível criar índice único em seo_knowledge.basename: {e}")
    ...
    def toggle_step(self, basename: str, step_index: int) -> Tuple[List[int], str]:
        # Toggle atômico via pipeline de agregação: remove o índice se presente, senão adiciona
        doc = self.collection.find_one_and_update(
            {"basename": basename},
            [
                {"$set": {
                    "user_implementation.completed_steps": {
                        "$let": {
                            "vars": {"completed": {"$ifNull": ["$user_implementation.completed_steps", []]}},
                            "in": {
                                "$cond": [
                                    {"$in": [step_index, "$$completed"]},
                                    {"$setDifference": ["$$completed", [step_index]]},
                                    {"$concatArrays": ["$$completed", [step_index]]}
                                ]
                            }
                        }
                    }
                ...
                {"$set": {
                    "user_implementation.status": {
                        "$let": {
                            "vars": {
                                "total": {"$size": {"$ifNull": ["$seo_knowledge.passo_a_passo_detalhado", []]}},
                                "done": {"$size": "$user_implementation.completed_steps"},
                                "current": {"$ifNull": ["$user_implementation.status", "pendente"]}
                            },
                            "in": {
                                "$cond": [
                                    {"$and": [{"$gt": ["$$total", 0]}, {"$eq": ["$$done", "$$total"]}]},
                                    "concluido",
                                    {"$cond": [
                                        {"$and": [{"$gt": ["$$done", 0]}, {"$eq": ["$$current", "pendente"]}]},
                                        "em_andamento",
                                        "$$current"
                                    ]}
                                ]
            ...
            projection={"user_implementation.completed_steps": 1, "user_implementation.status": 1}
        )
        if not doc:
            raise KeyError(f"Estratégia {basename} não encontrada")

        user_impl = doc.get("user_implementation", {})
        completed_list = sorted(user_impl.get("completed_steps", []))
        return completed_list, user_impl.get("status", "pendente")
...
class MongoAppConfigRepository(AbstractAppConfigRepository):
    ...
    def _validate_value(value: str, value_type: str) -> None:
        """Valida o valor contra o value_type declarado. Levanta ValueError se inválido."""
        try:
            if value_type == "int":
                int(value)
            ...
            raise ValueError(f"Valor '{value}' inválido para o tipo '{value_type}'")

    def update(self, key: str, value: str) -> Optional[AppConfig]:
        cfg = self.collection.find_one({"key": key, "editable": True}, {"_id": 0, "value_type": 1})
        if not cfg:
            return None
        self._validate_value(value, cfg.get("value_type", "string"))
...
class MongoProfilePostRepository(AbstractProfilePostRepository):
    ...
    def fix_video_post_types(self) -> int:
        """Migração única: corrige post_type para 'video' em posts que contenham URLs de vídeos/reels."""
        fixed = 0
        cursor = self.collection.find(
            {"post_type": {"$ne": "video"}},
            {"_id": 0, "post_id": 1, "post_url": 1, "media_items": 1}
        )
        for d in cursor:
